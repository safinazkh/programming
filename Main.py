...
class ResultDisplayer:
    def display_result(self, result, target_name, name_insert):
        try:
            ...
            print(f"An error occurred while displaying the result: {e}")
...
if __name__ == "__main__":
    try:
        ...
        for i in range(no_of_images):
            print(f"Enter the object {i + 1} image:")
            file_path = input()
            try:
                ...
                if img is None:
                    ...
                    exit()
                fileobject.append(file_path)
            except Exception as e:
                print(f"An error occurred while reading the image at {file_path}: {e}")
                exit()
        ...
        try:
            ...
            if Imagebase is None:
                print(f"Error: Unable to read the base image at {filename_ref}")
                exit()
        except Exception as e:
            print(f"An error occurred while reading the base image at {filename_ref}: {e}")
            exit()
